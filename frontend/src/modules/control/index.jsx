...
export default function ControlCenter() {
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#020617', fontFamily: "'Inter', sans-serif", color: '#e2e8f0', padding: 28 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
                ...
                <button onClick={fetchAll} style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 8, color: '#94a3b8', padding: '6px 14px', cursor: 'pointer', fontSize: '0.78rem' }}>
                    🔄 Refresh All
                </button>
            </div>

            {toast && (
                <div style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8, padding: '10px 16px', marginBottom: 20, fontSize: '0.85rem' }}>
                    {toast}
                </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20, marginBottom: 20 }}>
                ...
                <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 20 }}>
                    <h2 style={{ margin: '0 0 14px', fontSize: '0.95rem' }}>
                        ...
                        <span style={{ marginLeft: 8, color: appointments.length > 0 ? '#818cf8' : '#334155', fontWeight: 800 }}>
                            ({appointments.length})
                        </span>
                    </h2>
                    {loading && <p style={{ color: '#475569', fontSize: '0.8rem' }}>Loading...</p>}
                    {!loading && appointments.length === 0 && (
                        <p style={{ color: '#334155', fontSize: '0.8rem', fontStyle: 'italic', textAlign: 'center', padding: '20px 0' }}>
                            ✅ No pending appointment requests
                        </p>
                    )}
                    {appointments.map(a => (
                        <div key={a._id} style={{ background: '#020617', border: '1px solid #1e293b', borderRadius: 8, padding: '12px 14px', marginBottom: 10, borderLeft: '3px solid #818cf8' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                                <div>
                                    <span style={{ fontWeight: 700, fontSize: '0.85rem' }}>{a.patient_name}</span>
                                    <span style={{ color: '#475569', fontSize: '0.72rem', marginLeft: 8 }}>{a.phone}</span>
                                </div>
                            ...
                            <p style={{ color: '#94a3b8', fontSize: '0.78rem', margin: '0 0 10px', fontStyle: 'italic' }}>
                                "{a.message?.slice(0, 100)}"
                            </p>
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button onClick={() => confirmAppt(a)}
                                    style={{ flex: 1, padding: '6px 0', background: '#4f46e5', border: 'none', borderRadius: 6, color: '#fff', fontWeight: 700, fontSize: '0.78rem', cursor: 'pointer' }}>
                                    ✅ Confirm + Notify
                                </button>
                                <button onClick={() => dismissAppt(a._id)}
                                    style={{ padding: '6px 12px', background: '#1e293b', border: '1px solid #334155', borderRadius: 6, color: '#94a3b8', fontSize: '0.75rem', cursor: 'pointer' }}>
                                    Dismiss
                                </button>
                            </div>
                        </div>
                ...
                <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 20 }}>
                    <h2 style={{ margin: '0 0 14px', fontSize: '0.95rem' }}>
                        🤔 AI Escalations
                        <span style={{ marginLeft: 8, color: unmappedMessages.length > 0 ? '#fbbf24' : '#334155', fontWeight: 800 }}>
                    ...
                    {unmappedMessages.map(m => (
                        <div key={m._id} style={{ background: '#020617', border: '1px solid #1e293b', borderRadius: 8, padding: '12px 14px', marginBottom: 10, borderLeft: '3px solid #fbbf24' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                                <div>
                                    <span style={{ fontWeight: 700, fontSize: '0.85rem' }}>{m.patient_name}</span>
                                    <span style={{ color: '#475569', fontSize: '0.72rem', marginLeft: 8 }}>{m.phone}</span>
                                </div>
                                <span style={{ color: '#334155', fontSize: '0.65rem' }}>
                            ...
                            <p style={{ color: '#94a3b8', fontSize: '0.78rem', margin: '0 0 10px', fontStyle: 'italic', background: '#0f172a', padding: '6px 8px', borderRadius: '4px' }}>
                                "{m.message}"
                            </p>
                            <div style={{ display: 'flex', gap: 8 }}>
                ...
                <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 20 }}>
                    <h2 style={{ margin: '0 0 14px', fontSize: '0.95rem' }}>
                        🚨 Flagged Patients
                        <span style={{ marginLeft: 8, color: followups.length > 0 ? '#f87171' : '#334155', fontWeight: 800 }}>
                            ({followups.length})
                        </span>
                    </h2>
                    {loading && <p style={{ color: '#475569', fontSize: '0.8rem' }}>Loading...</p>}
                    ...
                    {followups.map(f => (
                        <div key={f._id} style={{ background: '#020617', border: '1px solid #1e293b', borderRadius: 8, padding: '10px 14px', marginBottom: 10, borderLeft: `3px solid ${RISK_COLOR[f.risk_label] || '#f87171'}` }}>
                            ...
                            {f.suggested_reason && (
                                <p style={{ color: '#475569', fontSize: '0.72rem', margin: '4px 0 0' }}>{f.suggested_reason}</p>
                            )}
                        </div>
                    ))}
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                ...
                <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 20 }}>
                    <h2 style={{ margin: '0 0 14px', fontSize: '0.95rem' }}>
                        🔎 Pending Care Gaps
                        <span style={{ marginLeft: 8, color: gaps.length > 0 ? '#fbbf24' : '#334155', fontWeight: 800 }}>
                            ({gaps.length})
                        </span>
                    </h2>
                    {loading && <p style={{ color: '#475569', fontSize: '0.8rem' }}>Loading...</p>}
                    ...
                    {!loading && gaps.length > 0 && (
                        <>
                            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                                <button onClick={selectAll} style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 6, color: '#94a3b8', padding: '4px 10px', cursor: 'pointer', fontSize: '0.72rem' }}>Select All</button>
                                <button onClick={clearAll} style={{ background: '#1e293b', border: '1px solid #334155', borderRadius: 6, color: '#94a3b8', padding: '4px 10px', cursor: 'pointer', fontSize: '0.72rem' }}>Clear</button>
                            </div>
                            <div style={{ maxHeight: 260, overflowY: 'auto', marginBottom: 14 }}>
                                {gaps.map(g => (
                                    <div key={g._id} onClick={() => toggleGap(g._id)}
                                        style={{
                                            background: selected.has(g._id) ? '#1e293b' : '#020617',
                                            border: `1px solid ${selected.has(g._id) ? '#4f46e5' : '#1e293b'}`,
                                        ...
                                        {g.message && (
                                            <p style={{ color: '#475569', fontSize: '0.72rem', margin: '4px 0 0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button onClick={() => bulkApprove(true)} disabled={!selected.size}
                                    style={{ flex: 1, padding: '9px 0', background: selected.size ? '#4f46e5' : '#1e293b', border: 'none', borderRadius: 8, color: '#fff', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer' }}>
                                    ✅ Approve + Send WhatsApp ({selected.size})
                                </button>
                                <button onClick={() => bulkApprove(false)} disabled={!selected.size}
                                    style={{ padding: '9px 14px', background: '#1e293b', border: '1px solid #334155', borderRadius: 8, color: '#94a3b8', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer' }}>
                                    Silent
                                </button>
                            </div>
                        </>
                ...
                <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 20 }}>
                    ...
                    <label style={{ color: '#94a3b8', fontSize: '0.75rem', display: 'block', marginBottom: 5 }}>Patient</label>
                    <select
                        value={sendMsg.patientId}
                        onChange={e => setSendMsg(p => ({ ...p, patientId: e.target.value }))}
                        style={{ width: '100%', background: '#1e293b', border: '1px solid #334155', borderRadius: 8, color: '#e2e8f0', padding: '8px 12px', marginBottom: 14, fontSize: '0.82rem', boxSizing: 'border-box' }}
                    >
                        <option value="">— Select patient —</option>
                        {patients.map(p => <option key={p._id} value={p._id}>{p.name}</option>)}
                    </select>
                    <label style={{ color: '#94a3b8', fontSize: '0.75rem', display: 'block', marginBottom: 5 }}>Message</label>
                    <textarea
                        rows={4}
                        placeholder="Type your message..."
                        value={sendMsg.text}
                        onChange={e => setSendMsg(p => ({ ...p, text: e.target.value }))}
                        style={{ width: '100%', boxSizing: 'border-box', background: '#1e293b', border: '1px solid #334155', borderRadius: 8, color: '#e2e8f0', padding: '8px 12px', fontSize: '0.82rem', resize: 'vertical', marginBottom: 14 }}
                    />
                    <button
                        onClick={sendManual}
